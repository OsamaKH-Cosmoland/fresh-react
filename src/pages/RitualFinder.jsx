...
import { addCartItem, readCart, subscribeToCart, writeCart } from "../utils/cartStorage.js";

const RITUAL_ARCHETYPES = {
  calm: {
    id: "calm",
...
export default function RitualFinder() {
  ...
  const resultProducts = useMemo(
    () => (result ? result.products.map((id) => PRODUCT_INDEX[id]).filter(Boolean) : []),
    [result]
  );
  ...
  const handleAddRitualToBag = () => {
    ...
    setCartItems((prev) => {
      ...
      result.products.forEach((productId) => {
        if (PRODUCT_INDEX[productId]) {
          updated = addCartItem(updated, { id: productId });
        }
      });
      writeCart(updated);
      return updated;
