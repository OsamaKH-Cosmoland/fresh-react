export const ar = {
  nav: {
    collection: "المجموعة",
    shop: "المتجر",
    favourites: "المفضلات",
    finder: "ابحث عن منتج",
    journal: "مدونتنا",
    guides: "دلائل الروتين",
    giftBuilder: "كوّن هدية",
    compare: "قارن",
    cart: "السلة",
    viewCart: "عرض السلة",
    orders: "الطلبات",
    account: "الحساب",
    audience: "الجمهور (المالك)",
    analytics: "التحليلات (المالك)",
    explore: "استكشف",
    ritualProfile: "ملف روتينك",
    ritualCoach: "مدرب الروتين",
    mobile: {
      sections: {
        shop: "المتجر",
        personalize: "تخصيص",
        utility: "أدوات",
      },
    },
  },
  currency: {
    label: "عملة العرض: {currency}",
  },
  meta: {
    titles: {
      home: "الرئيسية",
      shop: "المتجر",
      search: "البحث",
      favorites: "المفضلات",
      compare: "المقارنة",
      account: "حسابي",
      checkout: "الدفع",
      orders: "الطلبات",
      cart: "السلة",
      giftBuilder: "مصمم الهدايا",
      ritualFinder: "موجه الروتين",
      ritualCoach: "مدرب الروتين",
      ritualGuides: "دلائل الروتين",
      ritualPlanner: "منظم الروتين",
      onboarding: "إعداد ملف روتينك",
      stories: "قصص الروتين",
      storyDetail: "قصة الروتين",
      product: "المنتج",
      audience: "الجمهور",
      analytics: "التحليلات",
    },
    descriptions: {
      home: "تخلق NaturaGloss طقوساً مدروسة للعناية بالجسم والشعر، مرتكزة على النباتات والخطوات الحسية.",
      shop: "تصفح المنتجات والمجموعات المختارة حسب التركيز، الرائحة، والنية.",
      product: "اكتشف منتج NaturaGloss المصنوع يدوياً ليكون قلب روتينك اليومي.",
      guideList: "دلائل الروتين الهادئة التي تجمع بين المقالات والخطوات العملية.",
      guideDetail: "دليل روتيني خطوة بخطوة لمساعدتك على التباطؤ وطبقات العناية الهادئة.",
      giftBuilder: "كوّن صندوق هدايا مخصص مع منتجاتك المفضلة وملاحظاتك الخاصة.",
      finder: "أجب عن أسئلة مريحة لتكشف الروتين المناسب لاهتماماتك.",
      coach: "دع مدرب الروتين ينسق لك روتيناً مخصصاً حسب التركيز، الشدة، والرائحة.",
      account: "أدر روتيناتك المحفوظة، طلباتك، مراجعاتك، وتفضيلاتك بسهولة.",
      favorites: "احفظ الروتينات والمنتجات التي تروقك لتعود إليها في أي وقت.",
      compare: "قارن بين الروتينات، القوام، والأسعار لتختار الروتين الذي يناسبك.",
      checkout: "راجع السلة، أكد التفاصيل، وأكمل الدفع مع خيارات التوصيل المتكاملة.",
      orders: "استعرض طلباتك السابقة ومعلومات الشحن والمجموع بكل وضوح.",
      search: "ابحث في كل المنتجات، الدلائل، والروتينات عما يحتاجه روتينك الآن.",
      onboarding: "أنشئ ملف روتينك مع التركيز، التوقيت، الرائحة، والميزانية.",
      audience: "راجع جهات الاتصال والموافقات وسجل الطلبات للاستخدام الداخلي.",
      analytics: "لوحة تحليلات داخلية تُظهر الطلبات والمراجعات والتفاعلات المخزنة محلياً.",
    },
  },
  accessibility: {
    skipToContent: "انتقل إلى المحتوى الرئيسي",
    mainNavigation: "رئيسية",
    menu: {
      open: "افتح القائمة",
      close: "أغلق القائمة",
      title: "قائمة التنقل",
    },
    search: {
      suggestions: "اقتراحات البحث",
    },
    live: {
      addedToBag: "تمت إضافة {item} إلى السلة.",
    },
    favorite: {
      product: {
        add: "أضف {item} إلى المفضلات",
        remove: "أزل {item} من المفضلات",
      },
      bundle: {
        add: "أضف {item} إلى المفضلات",
        remove: "أزل {item} من المفضلات",
      },
      genericItem: "عنصر",
    },
    compare: {
      add: "أضف {item} إلى المقارنة",
      remove: "أزل {item} من المقارنة",
      genericItem: "عنصر",
    },
    tabs: {
      account: "علامات تبويب الحساب",
    },
    cart: {
      close: "أغلق نافذة السلة",
      decreaseQuantity: "قلل كمية {item}",
      increaseQuantity: "زد كمية {item}",
      removeItem: "أزل {item} من السلة",
    },
    variantSelector: "اختر نسخة",
  },
  lifecycle: {
    reasons: {
      firstVisit: "ترحيب الزيارة الأولى",
      subscriptionRefill: "تذكير بتجديد الاشتراك",
      softAbandonedCart: "سلة مهجورة بلطف",
      reviewReminder: "تذكير بالمراجعة",
      loyaltyThreshold: "معلومة الولاء",
      inactiveSinceOrder: "نفتقدك",
    },
    messages: {
      firstVisit: "مرحباً بك في NaturaGloss Rituals؛ دعنا نرشدك إلى رعاية هادئة وطبيعية.",
      subscriptionRefill: "سيُعاد تعبئة {subscriptionName} بتاريخ {refillDate}.",
      softAbandonedCart: "سلة روتينك تحتوي على {itemCount} منتجات بانتظار استكمالها.",
      reviewReminder: "مر {daysSince} يوماً على طلبك؛ {pending} روتيناً في انتظار مراجعتك.",
      loyaltyThreshold: "تبقى لك {pointsToNext} نقطة للرتبة التالية من الولاء.",
      inactiveSinceOrder: "مر {daysSinceLastOrder} يوماً منذ طلبك الأخير. دع روتينك ينعشك من جديد.",
    },
    actions: {
      refill: {
        title: "تذكير تعبئة الروتين",
        cta: "جهّز حقيبة الروتين",
        dismiss: "أغلق التذكير",
      },
    },
  },
  offline: {
    status: "غير متصل",
    message:
      "يبدو أنك غير متصل؛ الروتينات المحفوظة والدلائل وسلة التسوق ما تزال متاحة على هذا الجهاز.",
    checkoutWarning:
      "أنت غير متصل. يمكنك مراجعة السلة، لكن الدفع يتطلب اتصالاً.",
    installPrompt: "ثبت التطبيق",
  },
  sections: {
    ritualGuides: "دلائل الروتين",
    yourRituals: "روتينك",
    savedRituals: "روتين محفوظ",
    yourSavedRituals: "روتينك المحفوظ",
    yourFavourites: "المفضلات",
    recentlyViewed: "شوهد مؤخراً",
    products: "المنتجات",
    ritualBundles: "مجموعات الروتين",
    newHere: "جديد هنا؟",
    theRitualJournal: "سجل الروتين",
    ritualProfile: "ملف روتينك",
  },
  common: {
    item: "عنصر",
    items: "عناصر",
    moreItem: "عنصر إضافي",
    moreItems: "عناصر إضافية",
    lastUpdated: "آخر تحديث",
    recent: "حديثاً",
  },
  cardGrid: {
    toast: {
      title: "تمت الإضافة إلى سلتك",
      messagePrefix: "",
      messageSuffix: " أصبح الآن في سلتك. يمكنك مراجعة السلة في أي وقت.",
      itemFallback: "عنصر جديد",
      pairPrefix: "نسّقيه مع",
      pairSuffix: "لروتين متكامل.",
      addActionPrefix: "أضيفي",
      dismiss: "إغلاق الإشعار",
    },
    actions: {
      favorite: "إضافة للمفضلة",
      unfavorite: "إزالة من المفضلة",
    },
  },
  bundleCard: {
    compareAtLabel: "السعر المعتاد",
    savingsLabel: "توفر",
  },
  landing: {
    hero: {
      title: "فخامة مستوحاة من جوهر الطبيعة",
      subtitle:
        "انغمسي في عالم من السكينة والرقي؛ عناية طبيعية لمن يقدّر الجمال بروح.",
      imageAlt: "مجموعة NaturaGloss من العناية النباتية",
    },
    guides: {
      title: "روتينات تحريرية وملاحظات",
      subtitle: "مقالات هادئة وقراءات عميقة لكل طبقة من العناية.",
    },
    personalization: {
      subtitle: "محفوظة، محبوبة، أو أُعجِب بها مؤخراً — هذا المكان يتذكر كل لحظة.",
    },
    savedRituals: {
      subtitle: "أكملي الحزم التي احتفظتِ بها لليالي ممطرة أو صباحات مضيئة.",
    },
    favorites: {
      subtitle: "منتجات وروتينات علّمتِها للعودة إليها.",
    },
    recent: {
      subtitle: "تذكيرات لطيفة بما لفت انتباهك مؤخراً.",
    },
    guidance: {
      title: "ابدئي قصة روتينك",
      body:
        "استكشفي مُرشد الروتين لمسار مُنسّق، أو تصفحي المتجر لكل مكوّن يبعث على الهدوء.",
    },
    journal: {
      subtitle:
        "روتينات حسية مكتوبة — اقرئي كيف تقودك النباتات والأنفاس والنية في كل مساء وصباح وتوقّف.",
    },
    values: {
      title: "لماذا تختارين NaturaGloss",
      cards: {
        smallBatch: {
          title: "جودة الدُفعات الصغيرة",
          body:
            "مصنوعة يدوياً بكميات محدودة لضمان أن كل صابونة وبلسم طازج ومُعتنى به.",
        },
        transparency: {
          title: "شفافية المكونات",
          body: "كل المكونات مُعلنة بالكامل — بلا مواد مخفية، فقط نباتات نقية.",
        },
        standards: {
          title: "معايير مستوحاة من أوروبا",
          body:
            "تركيبات بإرشاد ممارسات السلامة والجودة في مستحضرات التجميل الأوروبية.",
        },
      },
    },
    legacy: {
      body:
        "ارتقي بروتينك اليومي بمكونات نباتية مغذية ولمسات مضيئة، مصنوعة على دفعات صغيرة لمن يبحث عن عناية ذات نية وإشراق.",
    },
    bundles: {
      subtitle: "مجموعات منسّقة تنسج المنتجات معاً لِلحظة راقية واحدة.",
    },
  },
  shop: {
    title: "تسوقي كل الروتينات والمنتجات",
    subtitle:
      "كل منتج وحزمة هنا. صفّي حسب التركيز أو النوع لتجدي الروتين المناسب ليومك.",
    filters: {
      focus: "التركيز",
      type: "النوع",
      activeHint: "تم تطبيق الفلاتر — تابعي التخصيص أو امسحي لرؤية الجميع.",
      idleHint: "تصفحي الكل أو استخدمي الفلاتر للتركيز على اهتمام محدد.",
    },
    sections: {
      products: {
        subtitle: "أساسيات فردية لتمتزج مع روتينك اليومي.",
      },
      bundles: {
        subtitle: "مجموعات متكاملة تجمع المنتجات لروتين أعمق.",
      },
    },
    empty: {
      title: "لا توجد نتائج مطابقة بعد",
      body:
        "عدّلي اختيارات الفلاتر لاكتشاف روتينات أخرى. نحفظ كل المنتجات والمجموعات هنا، ومسح الفلاتر يعيدها كلها.",
    },
  },
  searchPage: {
    title: "ابحثي عن الروتينات أو المنتجات أو التجارب",
    subtitlePrefix: "عرض “",
    subtitleSuffix: "”. حسّني الكلمات أو امسحي الحقل لرؤية الجميع.",
    subtitleDefault:
      "تصفحي كل الروتينات. استخدمي كلمات مفتاحية أو وسوم التركيز أو مُرشد الروتين.",
    sections: {
      products: {
        subtitle: "قطع فردية تُضاف إلى روتينك اليومي.",
      },
      guides: {
        title: "الأدلة",
        subtitle: "روتينات تحريرية ومقالات لعناية واعية.",
      },
      bundles: {
        subtitle: "روتينات متكاملة تجمع عناصرِك المفضلة.",
      },
      experiences: {
        title: "التجارب",
        subtitle: "إلهام وإرشاد لكل لحظة.",
      },
    },
    empty: {
      title: "لا توجد نتائج",
      body: "جرّبي كلمة أخرى أو زوري المتجر ومُرشد الروتين لمزيد من الإرشاد.",
    },
  },
  compare: {
    title: "قارني الروتينات والمنتجات",
    subtitle: "قارني التركيز والملمس والأسعار لاختيار الروتين الأنسب.",
    actions: {
      remove: "إزالة",
    },
    labels: {
      focus: "التركيز",
      benefits: "الفوائد الرئيسية",
      texture: "الملمس والرائحة",
      usage: "الاستخدام",
      anyRoutine: "أي روتين",
      included: "المحتويات",
      price: "السعر",
    },
    empty: {
      title: "لا توجد عناصر محددة",
      body: "أضيفي منتجات أو روتينات للمقارنة عبر زر المقارنة.",
    },
  },
  favorites: {
    title: "المفضلات المحفوظة",
    subtitle: "احفظي الروتينات والمنتجات التي تناسبك الآن.",
    sections: {
      products: {
        subtitle: "أساسيات فردية حفظتها للروتين القادم.",
      },
      bundles: {
        title: "المجموعات",
        subtitle: "روتينات محفوظة جاهزة للتركيب.",
      },
    },
    empty: {
      title: "لا توجد مفضلات بعد",
      body: "اضغطي على القلب لأي منتج أو روتين لحفظه هنا لاحقاً.",
    },
  },
  guides: {
    title: "دلائل الروتين",
    subtitle: "مقالات هادئة وروتينات ومعرفة لطيفة لكل طبقة.",
    detail: {
      notFoundTitle: "الدليل غير موجود",
      notFoundBody: "الروتين الذي تبحثين عنه لم يُكتب بعد.",
      related: {
        title: "أضيفي هذه الروتينات",
        subtitle: "اربطِي الدليل بمنتجات ومجموعات تُحييه.",
      },
    },
  },
  stories: {
    title: "مجلة الروتين",
    subtitle: "روتينات حسية بطيئة لدمج NaturaGloss في يومك.",
    intro:
      "كل روتين محفوظ في قصة دافئة مليئة بالعطر والأنفاس الهادئة لتبدئي عنايتك بنية.",
    labels: {
      date: "التاريخ",
      readTime: "مدة القراءة",
      minutes: "د",
      minutesRead: "د قراءة",
      tags: "وسوم القصة",
    },
    actions: {
      readStory: "اقرئي القصة",
      backToStories: "العودة إلى القصص",
    },
    detail: {
      notFoundTitle: "القصة غير موجودة",
      notFoundSubtitle: "ارجعي إلى مكتبة قصص الروتين.",
    },
  },
  productDetail: {
    backToCollection: "العودة إلى المجموعة",
    related: {
      title: "أكملي روتينك",
      subtitle: "مجموعات منسقة تتناسب طبيعياً مع هذا الجمال.",
    },
    sections: {
      madeFor: {
        title: "مناسب لـ",
      },
      routine: {
        title: "الروتين",
        subtitle: "تسلسل هادئ وحسي",
      },
      ingredients: {
        title: "إضاءة على المكونات",
      },
      sensory: {
        title: "التجربة الحسية",
      },
      pairs: {
        title: "يتناسق مع",
        subtitle: "أكملي الروتين",
      },
      faq: {
        title: "الأسئلة الشائعة",
        subtitle: "إجابات عن أسئلتك",
      },
    },
    aside: {
      label: "اطلبي هذا الروتين",
      hint: "مصمم لسكينة المساء ولمسة صباحية مضيئة.",
    },
  },
  ritualFinder: {
    title: "مُرشد الروتين",
    subtitle: "أجيبي على أسئلة هادئة وسننسّق الروتين المناسب لهذه الليلة.",
    intro:
      "نصغي لتفضيلات التركيز والوقت والرائحة لنوصي بالتوازن الدقيق بين التنظيف والعلاج والختام — ضمن مجموعاتنا المنسقة.",
    progressLabel: "الخطوة",
    actions: {
      back: "رجوع",
      continue: "متابعة",
      reveal: "اعرضي روتيني",
      reset: "أعيدي مُرشد الروتين",
    },
    results: {
      title: "نسّقنا هذا الروتين لكِ",
      subtitle: "أضيفي المجموعة الأنسب لإجاباتك.",
      alsoConsider: "قد يعجبك أيضاً",
      extrasSubtitle: "إضافات فردية تناسبها بشكل جميل.",
    },
    errors: {
      selectOption: "اختاري الإجابة الأقرب لليلة.",
    },
  },
  refillPlans: {
    frequencyLabel: "الإيقاع",
    createButton: "احفظ خطة التعبئة",
    savedMessage: "تم حفظها في خطط التعبئة.",
    errorMessage: "تعذّر حفظ الخطة الآن. حاول مرة أخرى لاحقاً.",
    orderLabel: "طلب {orderId}",
    frequencyLabels: {
      "30d": "كل 30 يوماً",
      "60d": "كل 60 يوماً",
      "90d": "كل 90 يوماً",
    },
  },
  analytics: {
    hero: {
      label: "رؤى المالك",
      title: "التحليلات",
      subtitle: "نظرة هادئة على الطلبات والمراجعات وإشارات الولاء المخزنة محلياً.",
    },
    sections: {
      keyMetrics: "المقاييس الرئيسية",
      topItems: "أفضل الروتينات والمنتجات",
      reviews: "المراجعات والتقييمات",
      favorites: "المفضلات والمقارنة",
      flowUsage: "استخدام المسارات",
    },
    cards: {
      ordersSummary: "ملخص الطلبات",
      loyalty: "ملخص الولاء",
      audience: "ملخص الجمهور",
      reviewSummary: "ملخص التقييمات",
      topRated: "أعلى المنتجات تقييماً",
      mostReviewed: "أكثر المنتجات مراجعة",
      favorites: "المفضلات",
      compare: "المقارنة",
      flowUsage: "استخدام المسارات المحلية",
      referrals: "الإحالات",
    },
    metrics: {
      ordersCount: "عدد الطلبات",
      totalRevenue: "إجمالي الإيرادات",
      aov: "متوسط قيمة الطلب",
      firstOrder: "أول طلب",
      lastOrder: "آخر طلب",
      repeatEstimate: "تقدير العملاء المتكرر",
      loyaltyPoints: "نقاط الولاء",
      audienceTotal: "إجمالي جهات الاتصال",
      audienceWithOrders: "جهات اتصال بصرف طلبات",
      referralOrders: "طلبات الإحالة",
      referralCredit: "الرصيد الممنوح",
      globalRating: "متوسط التقييم",
    },
    labels: {
      loyaltyHint: "سجل الولاء الحالي مأخوذ من التخزين المحلي.",
      loyaltyTierUnavailable: "بيانات المستوى غير متوفرة بعد.",
      reviewCount: "{count} إجمالي التقييمات",
      favoritesHint: "العناصر المفضلة محفوظة محلياً.",
      compareHint: "العناصر المضافة للمقارنة.",
      referralTopCodes: "أفضل رموز الإحالة",
      referralCode: "الرمز",
      referralOrders: "الطلبات",
      referralCredit: "الرصيد الممنوح",
      notAvailable: "غير متوفر حالياً",
      type: {
        product: "منتج",
        bundle: "مجموعة",
        gift: "هدية",
      },
    },
    table: {
      name: "الاسم",
      type: "النوع",
      quantity: "الكمية",
      revenue: "الإيرادات",
      rating: "التقييم",
      reviews: "مراجعات",
    },
    flow: {
      finder: "موجه الروتين",
      coach: "مدرب الروتين",
      gift: "منشئ الهدايا",
      statusUsed: "مراقب",
      statusUntracked: "لم تتم مراقبته بعد",
    },
  },
  reviews: {
    eyebrow: "تجارب مجتمعنا",
    sectionTitle: "تقييمات العملاء",
    sectionSubtitle: "تجارب يشاركها مجتمع NaturaGloss.",
    loading: "جارٍ تحميل المراجعات…",
    summary: {
      reviewLabelSingular: "مراجعة",
      reviewLabel: "تقييمات",
      noReviews: "لا توجد تقييمات بعد",
      noRating: "لم يتم تقييمه بعد",
      verifiedHint: "مشتريات معتمدة فقط",
      ratingAriaLabel: "تم التقييم بـ {rating} من 5 من {count} {reviewLabel}",
    },
    list: {
      filterLabel: "فلترة حسب التقييم",
      filterOptions: {
        all: "كل التقييمات",
        five: "5 نجوم",
        fourPlus: "4 نجوم فأكثر",
        threePlus: "3 نجوم فأكثر",
      },
      verifiedOnly: "عرض المشتريات المعتمدة",
      verifiedBadge: "شراء مؤكد",
      anonymous: "مجهول",
      photoAlt: "صورة العميل",
      empty: "لا توجد مراجعات بعد – كن أول من يشارك تجربته.",
    },
    form: {
      heading: "شارك روتينك",
      subtitle: "أخبرنا كيف جعلتك هذه التجربة تشعر.",
      placeholders: {
        body: "احكي لنا عن روتينك...",
      },
      labels: {
        rating: "التقييم",
        title: "عنوان (اختياري)",
        body: "تجربتك",
        photoUrl: "رابط صورة (اختياري)",
        reviewerName: "الاسم (اختياري)",
        star: "نجمة",
        stars: "نجوم",
      },
      submit: "إرسال التقييم",
      submitting: "جارٍ الإرسال...",
      thankYou: "شكراً لمشاركة روتينك!",
      success: "تم نشر تقييمك – شكراً لك.",
      errors: {
        body: "شاركنا ببضع كلمات عن هذا الروتين.",
        generic: "حدث خطأ، نرجو المحاولة لاحقاً.",
      },
    },
    errors: {
      load: "تعذّر تحميل المراجعات.",
      submit: "تعذّر إرسال المراجعة.",
    },
  },
  variants: {
    summary: {
      available: "متوفر بـ {count} نغمات",
    },
  },
  cta: {
    addToBag: "أضف إلى السلة",
    addRitualToBag: "أضف الروتين إلى السلة",
    viewDetails: "عرض التفاصيل",
    findYourRitual: "اختر روتينك",
    viewAllGuides: "عرض كل الأدلة",
    readGuide: "اقرأ الدليل",
    resumeRitual: "استكمال الروتين",
    viewRitual: "عرض الروتين",
    helpChooseRitual: "ساعدني في اختيار روتين",
    browseRitualsProducts: "تصفح الروتينات والمنتجات",
    browseShop: "تصفح المتجر",
    openRitualFinder: "افتح موجه الروتين",
    goToShop: "انتقل إلى المتجر",
    open: "افتح",
    shopCollection: "تسوق المجموعة",
    findMyProduct: "ابحث عن منتجك",
    buildAGift: "كوّن هدية",
    clearAll: "مسح الكل",
    backToShop: "عودة إلى المتجر",
    clearFilters: "مسح الفلاتر",
    resetFilters: "إعادة تعيين الفلاتر",
    continueShopping: "تابع التسوق",
    clearBag: "إفراغ السلة",
    proceedToCheckout: "المتابعة إلى الدفع",
    viewBag: "عرض السلة",
    createRitualProfile: "أنشئ ملف روتينك",
  },
  cart: {
    summary: {
      title: "ملخص الطلب",
      items: "العناصر",
      subtotal: "المجموع الفرعي",
      discount: "الخصم",
      total: "الإجمالي",
      giftCredit: "رصيد الهدايا",
    },
    drawer: {
      title: "سلتك",
      empty: "سلتك فارغة حالياً.",
      saved: {
        title: "الروتينات المحفوظة",
        subtitle: "احفظي سلتك الحالية للعودة إليها لاحقاً.",
        placeholder: "سمّي هذا الروتين",
        saveAction: "حفظ",
        load: "تحميل",
        loaded: "تم التحميل",
        delete: "حذف",
        empty: "لا توجد روتينات محفوظة بعد.",
      },
      save: {
        success: "تم الحفظ. يمكنك تحميله لاحقاً.",
        errors: {
          missingName: "يرجى إدخال اسم للروتين.",
          empty: "أضيفي منتجات قبل الحفظ.",
        },
      },
    },
    promo: {
      label: "رمز التوفير",
      placeholder: "أدخل الرمز",
      apply: "تطبيق",
      remove: "إزالة الرمز",
      success: "تم تطبيق الرمز",
      invalid: "الرمز غير معتمد",
      notApplicable: "الرمز لا ينطبق على روتينك الحالي الآن",
      removed: "تم إزالة الرمز",
      youSave: "توفر {amount}",
      freeShipping: "توصيل مجاني",
    },
    giftCredit: {
      title: "بطاقة هدية أو رصيد المتجر",
      help: "أضف الرمز قبل التوجه للدفع لتطبيق الرصيد على الطلب.",
      placeholder: "أدخل رمز البطاقة أو الرصيد",
      inputLabel: "رمز بطاقة الهدية أو الرصيد",
      apply: "تطبيق",
      remove: "إزالة الرصيد",
      appliedLabel: "الرمز المطبق: {code}",
      errors: {
        not_found: "الرمز غير موجود",
        exhausted: "هذا الرصيد استُهلك بالكامل.",
        invalid: "لا يمكن استخدام هذا الرمز الآن.",
      },
    },
  },
  loader: {
    yourBag: "سلتك",
    gatheringYourRituals: "نجمع روتيناتك",
    ritualFinder: "موجه الروتين",
    craftingYourRitualPath: "ننسج مسار روتينك",
    search: "بحث",
    aligningEveryRitualNote: "ننسق كل نغمة روتينية",
    compare: "مقارنة",
    balancingYourRituals: "مواءمة روتيناتك",
    yourFavourites: "مفضلاتك",
    gatheringYourCalmPicks: "نجمع اختيارات الهدوء",
    shop: "المتجر",
    curatingTheRitualCatalog: "نرتب كتالوج الروتين",
    ritualGuides: "دلائل الروتين",
    sourcingMindfulGuides: "نستقصي دلائل مدروسة",
    giftBuilder: "مصمم الهدايا",
    assemblingYourCustomBox: "نركب صندوقك المخصص",
    ritualProfile: "نصوغ ملف روتينك",
    ordersHistory: "سجل الطلبات",
  },
  giftBuilder: {
    stepperLabel: "خطوات مصمم الهدايا",
    boxSelectorLabel: "اختر تصميم الصندوق",
    actions: {
      addProduct: "أضف {name} إلى صندوقك",
      removeProduct: "أزل {name} من صندوقك",
      addAddOn: "أضف {name} إلى إضافاتك",
      removeAddOn: "أزل {name} من إضافاتك",
    },
    comingSoon: {
      title: "كوّن هديتك",
      subtitle: "قريباً",
      body:
        "نعمل على صقل تجربة مصمم الهدايا لتصل طقوس العناية المنسقة كحزم ساحرة. ترقبوا الإطلاق الأول قريباً.",
      note: "في الوقت الحالي، تصفحي المجموعة وأضيفي مفضلاتك إلى طقم طقوسي.",
      action: "تصفحي المجموعة",
    },
  },
  search: {
    placeholder: "ابحث عن الروتينات والمنتجات...",
    noResults: "لا توجد نتائج بعد. جرّب مصطلحاً آخر أو اضغط إدخال لرؤية الكل.",
    seeAll: "عرض كل النتائج",
  },
  filters: {
    allRitualsProducts: "جميع الروتينات والمنتجات",
    products: "منتجات",
  },
  sort: {
    label: "ترتيب",
    default: "الترتيب الافتراضي",
    recommended: "مقترح لك",
  },
  onboarding: {
    hero: {
      title: "أنشئ ملف روتينك",
      subtitle:
        "شارك تفضيلات هادئة حتى يكيّف NaturaGloss كل لحظة هبوط ومسار موجه الروتين لطريقة هدوءك.",
    },
    steps: {
        concerns: {
        title: "أين يتركز اهتمامك",
        description: "اختر الروتين أو المناطق التي ترغب في تنعيمها.",
      },
      time: {
        title: "متى تفضل روتينك",
        description: "متى ترغب في اللحظات الاسترخائية؟",
      },
      scent: {
        title: "نغمة الرائحة",
        description: "اختر العائلة العطرية التي تشعرك بالهدوء.",
      },
        budget: {
        title: "راحة الميزانية",
        description: "شارك كيف تميل عند اقتناء الروتينات.",
      },
    },
    options: {
      concerns: {
        bodyHydration: {
          label: "ترطيب الجسم وتوهج",
          helper: "طبقات زيوت وبارفات للأذرع والساقين والكتفين.",
        },
        hairGrowth: {
          label: "نمو الشعر وقوته",
          helper: "دعم فروة الرأس بالزيوت والسيرومات المغذية.",
        },
        handsLips: {
          label: "العناية باليدين والشفاه",
          helper: "لمسات سريعة للراحة على الكفين والشفاه.",
        },
      },
        time: {
        morning: {
          label: "صباح مريح",
          helper: "روتين سريع يجهّز البشرة لليوم.",
        },
        evening: {
          label: "سهرات المساء",
          helper: "لحظات بطيئة تغلق اليوم بلطف.",
        },
        both: {
          label: "صباحاً ومساءً",
          helper: "روتينات يمكن تكرارها عند الشروق والغسق.",
        },
        express: {
          label: "إعادة سريعة",
          helper: "أقل من ١٠ دقائق مع شعور بالترف.",
        },
      },
      scent: {
        softFloral: {
          label: "زهري ناعم",
          helper: "نيرولي، ياسمين، وبتلات بودرية.",
        },
        fresh: {
          label: "انتعاش نقي",
          helper: "نغمات خضراء، حمضيات، وندى شفاف.",
        },
        warm: {
          label: "توّه دافئ",
          helper: "عنبر، راتنج، وأخشاب دافئة.",
        },
        unscented: {
          label: "راحة بدون عطر",
          helper: "ملمس نقي دون إضافات عطرية.",
        },
      },
        budget: {
          valueFocused: {
            label: "قيمة مدروسة",
            helper: "مجموعات سخية ضمن متناول اليد.",
          },
          premium: {
            label: "روتين فاخر",
            helper: "تجارب بطبعات صغيرة ولمسات فاخرة.",
          },
        },
    },
    actions: {
      back: "عودة",
      skipFlow: "تخطي الآن",
      skipStep: "تخطي هذا السؤال",
      continue: "متابعة",
      finish: "حفظ التفضيلات",
      edit: "تحديث ملف روتينك",
    },
    completion: {
      title: "الآن أنت جاهز",
      subtitle:
        "يُخزن ملف روتينك محلياً ويُوجه ما تراه في الصفحة وموجه الروتين.",
      body: "عد في أي وقت لتنعيم ما تهتم به.",
      ctas: {
        landing: "العودة للصفحة الرئيسية",
        finder: "افتح موجه الروتين",
      },
    },
    newsletter: {
      heading: "حافظ على تواصل الروتين",
      body: "شارك بريدًا إلكترونيًا لنرسل تحديثات ونصائح مبنية على ملفك.",
      emailLabel: "البريد الإلكتروني (اختياري)",
      emailPlaceholder: "you@naturagloss.com",
      consentLabel: "أود النصائح والاقتراحات حسب ملفي.",
      helper: "أكمل الملف لنُسجل ملاحظات الروتين.",
      success: "شكراً—تم حفظ تحديثات ملفك.",
      errors: {
        invalidEmail: "استخدم بريداً إلكترونياً صالحاً.",
        general: "حدث خطأ أثناء حفظ بريدك.",
      },
    },
    personalization: {
      title: "ملف روتينك",
      description: "نبرز الروتينات التي تتوافق مع تركيزك وتوقيتك.",
      updateLink: "تعديل الملف",
      reasons: {
        matchesConcern: "يتماشى مع تركيزك الحالي",
        matchesTime: "يناسب توقيت روتينك المفضل",
        matchesScent: "يتوافق مع نغمة عطرك المختارة",
        withinBudget: "يتناسب مع ميزانيتك",
        premiumBudget: "خيار فاخر ضمن راحتك",
        inFavorites: "أضفته للمفضلات",
        recentlyViewed: "اطّلعت عليه مؤخراً",
        purchasedBefore: "سبق أن اشتريته",
        reviewedByYou: "كتبت عنه مراجعة",
        highGlobalRating: "محبوب من الجميع",
        matchesContextFocus: "متماشي مع تركيزك الحالي",
      },
      recommendedLabel: "موصى به لك",
    },
    hint: "استناداً لملف روتينك، نميل لهذه الروتينات الهادئة.",
    progress: {
      step: "الخطوة {current} من {total}",
    },
    errors: {
      selectConcern: "اختر تركيزاً واحداً على الأقل أو تخطّ هذه الخطوة.",
    },
  },
  checkout: {
    hero: {
      title: "إرشاد الدفع",
      subtitle: "أربع خطوات ناعمة لمراجعة روتينك وتأكيد توصيله.",
      meta: "لا مفاجآت، فقط عناية مدروسة.",
    },
    steps: {
      contact: "الاتصال والعنوان",
      shipping: "الشحن",
      payment: "الدفع",
      review: "المراجعة والطلب",
    },
    sections: {
      contact: "تفاصيل الاتصال والعنوان",
      contactHelp: "أخبرنا كيف نصل إليك لنصل روتينك برفق.",
      shipping: "اختر طريقة شحن",
      shippingHelp: "عيّن وتيرة التوصيل التي تناسب توقيتك الروتيني.",
      payment: "اختر وسيلة دفع",
      paymentHelp: "اختر طريقة الدفع المناسبة لإكمال الطلب.",
      review: "راجع الطلب",
      reviewHelp: "تأكد من المجموعات، الرسوم والعنوان قبل الحجز.",
    },
    newsletterConsent: "أبقني مطلعاً على الطقوس والعروض الجديدة.",
    fields: {
      fullName: "الاسم الكامل",
      email: "البريد الإلكتروني",
      phone: "رقم الهاتف",
      country: "الدولة",
      city: "المدينة",
      street: "الشارع والرقم",
      postalCode: "الرمز البريدي (اختياري)",
      cardNumber: "رقم البطاقة",
      cardExpiry: "تاريخ الانتهاء (MM/YY)",
      cardCvc: "رمز الأمان",
    },
    shipping: {
      addressTitle: "عنوان التوصيل",
    },
    shippingOptions: {
      standard: {
        label: "توصيل عادي",
        description: "٣-٥ أيام مع متابعة الكونسييرج.",
        eta: "٣-٥ أيام",
      },
      express: {
        label: "توصيل سريع",
        description: "١-٢ يوم بترتيب فائق.",
        eta: "١-٢ يوم",
      },
      local: {
        label: "استلام محلي",
        description: "استلم من الاستوديو في نطاق محدود.",
        eta: "نفس اليوم",
      },
    },
    payment: {
      methods: {
        cod: "الدفع عند الاستلام",
        instapay: "إنستا باي",
        vodafone: "فودافون كاش",
      },
      transferNote:
        "ابعت الحساب علي رقم 01273922868 بأسم اسامة و ابعت سكرين شوت بالحواله لرقم الواتس 010154291569 او علي صفحه الانستا بتاعتنا",
    },
    validation: {
      required: "هذا الحقل مطلوب.",
      emailInvalid: "أدخل بريداً إلكترونياً صحيحاً.",
      cardNumber: "رقم البطاقة يجب أن يكون ١٦ رقماً.",
      cardExpiry: "استخدم الصيغة MM/YY.",
      cardCvc: "أدخل ٣ أو ٤ أرقام.",
      orderSubmit:
        "تعذر إرسال الطلب إلى الخادم. تأكد أن واجهة API تعمل ثم حاول مرة أخرى.",
    },
    actions: {
      next: "الخطوة التالية",
      back: "رجوع",
      placeOrder: "قدم الطلب",
      submittingOrder: "جارٍ إرسال الطلب...",
    },
    stepperLabel: "خطوات الدفع",
    review: {
      labels: {
        gift: "هدية",
        bundle: "مجموعة",
        subtotal: "المجموع الفرعي",
        discount: "الخصم",
        shipping: "الشحن",
        total: "الإجمالي",
        promoCode: "رمز التوفير",
        shippingMethod: "طريقة الشحن",
        payment: "طريقة الدفع",
        address: "عنوان التوصيل",
        giftCredit: "رصيد الهدايا",
      },
    },
    defaults: {
      country: "مصر",
    },
    empty: {
      title: "سلتك خفيفة.",
      body: "أضف روتيناً لنكمل عملية الدفع.",
    },
    confirmation: {
      title: "شكراً لك",
      subtitle: "روتينك في الطريق مع لمسة حانية.",
      summary: "أضفت {itemCount} عناصر بإجمالي {total}.",
      refillTitle: "أبقِ هذا الروتين على التكرار",
      refillBody: "احفظ {itemCount} منتجاً لنذكّرك بتعبئته على إيقاع هادئ.",
      refillButton: "احفظ خطة التعبئة",
      loyaltyEarned: "لقد كسبت {points} نقطة طقسية مع هذا الطلب.",
      loyaltyStatus: "حالتك: {tier}.",
      loyaltyTotal: "لديك الآن {points} نقطة طقسية.",
      loyaltyProgress: "احصل على {points} نقطة أخرى للوصول إلى {nextTier}.",
      giftCodesTitle: "رموز الهدايا",
      keepCodesSafe: "احفظ هذه الرموز بأمان وشاركها مباشرة مع من أهديتهم إياها.",
      creditUsed: "استخدمت {amount} من رصيد الهدايا (الرمز: {code}).",
      referralHint: "شارك هذا الرابط مع صديق ليكسب رصيداً في المتجر.",
      viewOrders: "عرض طلباتي",
    },
    cta: {
      start: "ابدأ الدفع",
    },
  },
  ordersHistory: {
    title: "سجل الطلبات",
    subtitle: "راجع كل روتينك التي تأكدت منها.",
    empty: "لم تطلب أي روتين بعد.",
    labels: {
      orderId: "طلب",
      itemCount: "{count} عناصر",
      view: "عرض التفاصيل",
      hide: "إخفاء التفاصيل",
      gift: "هدية",
      bundle: "مجموعة",
    },
    detail: {
      shippingMethod: "طريقة الشحن",
      address: "عنوان التسليم",
      payment: "ملخص الدفع",
      discount: "الخصم",
      promoCode: "رمز التوفير",
      last4: "بطاقة تنتهي بـ {last4}",
      items: "عناصر الطلب",
    },
    loyalty: {
      label: "الولاء",
      status: "المستوى الحالي: {tier}",
      points: "عدد النقاط: {points}",
      nextTier: "احصل على {points} نقطة لتصل إلى {tier}.",
      maxTier: "أنت في أعلى مستوى من الولاء.",
    },
  },
  ritualCoach: {
    loader: {
      title: "مدرب الروتين",
      subtitle: "يختار روتينات هادئة تتناغم مع تفضيلاتك.",
    },
    hero: {
      title: "اسأل مدرب الروتين",
      subtitle: "رفيق قواعدي يرشد روتينك القادم بلطف وتركيز بما يتوافق مع ما تهتم به.",
    },
    profileRecap: {
      template:
        "ذكرتَ أنك تهتم بروتين {focus}، تفضل توقيت {time}، تعشق نغمات {scent}، وتتحمل ميزانية {budget}.",
      empty: "أنشئ ملف روتين حتى ينسجم المدرب مع ما يهمك.",
    },
    controls: {
      intensity: {
        label: "ما حدة الروتين التي تسعى إليها؟",
        helper: "اختر ما بين بسيطة، متوازنة، أو متكاملة حسب مزاجك.",
        options: {
          minimal: "بسيطة",
          balanced: "متوازنة",
          indulgent: "متكاملة",
        },
      },
      focus: {
        label: "هل ترغب في تركيز الشعر أم الجسم الآن؟",
        helper: "حدد المنطقة التي تريد أن يسلط عليها المدرب الضوء.",
        clear: "إزالة التركيز",
      },
    },
    sections: {
      main: "روتينك الرئيسي",
      lighter: "نسخة أخف",
      treats: "لمسات تدليل",
    },
    explanations: {
      main: "يتماشى مع توقيت {time} وتركيز {focus} ليمنحك لمسة {intensity}.",
      lighter: "نسخة أخف من روتين {intensity} تحافظ على {focus} وتوقيت {time}.",
      treats: "أضف لمسات {scent} عند اشتياقك لنهاية {mood}.",
    },
    emptyState: "يحتاج المدرب إلى ملف روتين لتجهيز تطابق.",
    treats: {
      empty: "عد بعد تحديد تركيز لتظهر اقتراحات التدليل.",
    },
    savedCartName: {
      base: "روتين المدرب",
    },
    savedCartMessage: {
      success: "تم حفظ روتين المدرب.",
      empty: "أضف روتيناً قبل الحفظ.",
    },
    cta: {
      createProfile: "أنشئ ملف الروتين",
      addFullRitual: "أضف الروتين الكامل للسلة",
      saveRitual: "احفظ هذا الروتين",
      askAgain: "اسأل المدرب مجدداً",
      askCoach: "اسأل مدرب الروتين",
      refineWithCoach: "نقّح بالتعاون مع مدرب الروتين",
    },
  },
  account: {
    hero: {
      title: "مساحة الروتين الخاصة بي",
      subtitle: "مكان هادئ لتفضيلاتك، طلباتك، الروتينات المحفوظة، مفضلاتك، وتقييماتك.",
    },
    tabs: {
      profile: "الملف",
      orders: "الطلبات",
      refillPlans: "خطط التعبئة",
      credits: "الائتمانات وبطاقات الهدايا",
      referrals: "الإحالات",
      saved: "روتين محفوظ",
      favorites: "المفضلات",
      reviews: "التقييمات",
    },
    profile: {
      focusLabel: "التركيز",
      timeLabel: "الوقت المفضل",
      scentLabel: "الرائحة",
      budgetLabel: "راحة الميزانية",
      emptyValue: "لم يتم التحديد بعد",
      hint: "توجه هذه التفضيلات موجه البنية والروتين ومدرب الروتين لتبقى على نفس النغمة.",
      edit: "تعديل ملف الروتين",
    },
    region: {
      title: "المنطقة والعملات",
      description: "اختر العملة التي تود رؤية الأسعار بها؛ السعر الإجمالي يبقى بالعملة الأساسية.",
    },
    loyalty: {
      title: "الولاء",
      totalPoints: "مجموع النقاط الروتينية",
      lastOrder: "آخر طلب",
      starting: "أكمل طلبك الأول لتبدأ في كسب نقاط الروتين.",
      nextTier: "احصل على {points} نقطة إضافية لتصل إلى {tier}.",
      maxTier: "أنت في أعلى مستوى من الولاء.",
      tiers: {
        bronze: {
          label: "رتبة البرونز",
          perks: "ترحيب لطيف يفتح باب عالم NaturaGloss الروتيني.",
        },
        silver: {
          label: "رتبة الفضة",
          perks: "عناية أعمق ووصول مبكر لتجارب وروتينات جديدة.",
        },
        gold: {
          label: "رتبة الذهب",
          perks: "عناية مميزة وأولوية في طقوس الموسم الخاص.",
        },
        platinum: {
          label: "رتبة البلاتين",
          perks: "مكانة رفيعة مع أرحب مستويات العناية.",
        },
      },
    },
    orders: {
      emptyTitle: "لا توجد طلبات بعد",
      emptyBody: "عند إتمام طلب ستجده هنا مع تفاصيل التوصيل.",
    },
    credits: {
      heading: "الائتمانات وبطاقات الهدايا",
      description: "تظهر هنا بطاقات الهدايا أو الأرصدة اليدوية المتاحة لطلبك القادم.",
      emptyTitle: "لا توجد أرصدة حالياً",
      emptyBody: "ستُضاف هنا الأرصدة التي تكسبها أو تهديها بعد كل طلب.",
      remaining: "الرصيد المتبقي: {amount}",
      statuses: {
        active: "فعال",
        exhausted: "مستنفد",
        disabled: "معطل",
      },
      createdAt: "أضيف في {date}",
    },
    referrals: {
      heading: "الإحالات",
      description: "شارك رابطك لدعوة الأصدقاء وكسب رصيد المتجر.",
      linkLabel: "رابط الإحالة الخاص بك",
      copyLink: "نسخ الرابط",
      copied: "تم نسخ الرابط",
      copyError: "تعذر نسخ الرابط حالياً.",
      stats: {
        orders: "طلبات من الرابط",
        credit: "إجمالي رصيد الإحالة",
      },
      form: {
        nameLabel: "الاسم (اختياري)",
        emailLabel: "البريد الإلكتروني (اختياري)",
        save: "حفظ معلومات الإحالة",
        saved: "تم حفظ معلومات الإحالة",
      },
      list: {
        title: "الإحالات الأخيرة",
        empty: "لا توجد طلبات محالة بعد.",
        orderLabel: "طلب {orderId}",
        credited: "الرصيد: {amount}",
      },
    },
    saved: {
      empty: "احفظ روتينًا لتعود إليه متى شئت.",
      load: "تحميل الروتين في السلة",
      delete: "حذف الروتين",
      refillTitle: "حدد وتيرة تعبئة لطيفة",
      refillBody: "احفظ هذا الروتين واختر تكراراً ناعماً لنذكّرك قبل موعد التعبئة.",
      refillButton: "احفظ خطة التعبئة",
    },
    favorites: {
      empty: "اضغط على القلب لأي روتين أو منتج لتثبيته هنا.",
      remove: "إزالة من المفضلات",
      products: "منتجات",
      bundles: "مجموعات",
    },
    reviews: {
      heading: "تقييماتي",
      empty: "لم تشارك بعد تجربة روتينية.",
      verifiedLabel: "روتين موثق",
    },
    refillPlans: {
      heading: "خطط التعبئة",
      description: "تذكيرات هادئة وتجهيز للروتين حتى تعيد طلبيتك دون ضجيج.",
      emptyTitle: "لا توجد خطط تعبئة بعد",
      emptyBody: "احفظ طلباً أو روتيناً وسيظهر هنا كخطة تعبئة.",
      frequencyLabel: "التكرار",
      nextRefillLabel: "التعبئة التالية",
      nextRefillNone: "—",
      placeholderLabel: "سمِّ هذا الروتين",
      statuses: {
        active: "نشط",
        paused: "متوقف مؤقتاً",
        cancelled: "ملغى",
      },
      actions: {
        refillNow: "أعد تعبئته الآن",
        pause: "أوقف الخطة",
        resume: "استأنف الخطة",
        cancel: "ألغِ الخطة",
        delete: "احذف الخطة",
      },
    },
  },
  newsletter: {
    heading: "تابعنا في تدوينتك الروتينية",
    subtitle: "احصل على تحديثات هادئة وروتينات وعروض مصممة لروحك.",
    helper: "نشارك فقط ملاحظات هادئة عن الطقوس والعروض.",
    form: {
      emailLabel: "البريد الإلكتروني",
      emailPlaceholder: "you@naturagloss.com",
      consentLabel: "أخبرني بالنصائح والعروض",
      submit: "انضم للقائمة",
    },
    success: "تمت الإضافة — ستصلك نغمات روتينية هادئة قريباً.",
    errors: {
      invalidEmail: "يرجى إدخال بريد إلكتروني صالح.",
      general: "تعذر الحفظ الآن. أعد المحاولة لاحقاً.",
    },
  },
  audience: {
    hero: {
      title: "الجمهور",
      subtitle: "جهات الاتصال، الموافقات، وسياق الطلبات لعرض داخلي.",
    },
    filters: {
      channel: "قناة الموافقة",
      allChannels: "كل القنوات",
      orders: {
        label: "نشاط الطلب",
        all: "كل الجهات",
        withOrders: "له طلبات",
        withoutOrders: "لم يطلب بعد",
      },
    },
    actions: {
      exportJson: "تصدير JSON",
      exportCsv: "تصدير CSV",
    },
    table: {
      email: "البريد الإلكتروني",
      locale: "اللغة",
      concerns: "الاهتمامات",
      orders: "الطلبات",
      lastOrder: "آخر طلب",
      consents: "الموافقات",
      created: "أنشئ في",
      updated: "آخر تحديث",
    },
    emptyState: "لا توجد جهات اتصال محفوظة حتى الآن.",
    consentChannels: {
      newsletter: "النشرة الإخبارية",
      productUpdates: "تحديثات المنتجات",
      offers: "عروض وترويجات",
      research: "أبحاث",
    },
    sources: {
      newsletter_form: "نموذج النشرة",
      checkout: "الشراء",
      onboarding: "إنشاء الملف",
      account: "الحساب",
    },
  },
  mobile: {
    sections: {
      shop: "المتجر",
      personalize: "شخصي",
      utility: "الأدوات",
    },
  },
} as const;
