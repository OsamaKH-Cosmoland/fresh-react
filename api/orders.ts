...
function sanitizeOrderPayload(rawBody: any): Order {
...
const notifyTelegram = async (order: Partial<Order>) => {
  try {
...
async function listOrders(limit: number, repo?: OrdersRepository) {
  const store = repo ?? (await resolveOrdersRepository()).store;
  const docs = await store.list(limit);
  return docs.map(cleanOrderDoc);
}

async function createOrder(rawBody: any, repo?: OrdersRepository, emailProvider?: EmailProvider) {
  const store = repo ?? (await resolveOrdersRepository()).store;
...
async function updateOrderStatus(id: string, status: string, repo?: OrdersRepository) {
  const store = repo ?? (await resolveOrdersRepository()).store;
  const updated = await store.updateStatus(id, status);
  return updated ? cleanOrderDoc(updated) : null;
}

async function notifyTelegramTest() {
  const { token, chatId } = getTelegramConfig();
  if (!token || !chatId) {
