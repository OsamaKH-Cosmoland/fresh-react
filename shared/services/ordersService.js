...
const roundCurrency = (value) => Number.isFinite(value) ? Math.round((value + Number.EPSILON) * 100) / 100 : 0;
...
const getTelegramConfig = () => {
    ...
    const envToken = resolveFromCandidates(process.env.TELEGRAM_BOT_TOKEN || "", process.env.VITE_TELEGRAM_BOT_TOKEN || "", process.env.REACT_APP_TELEGRAM_BOT_TOKEN || "", process.env.NG_TELEGRAM_BOT_TOKEN || "");
    const envChat = resolveFromCandidates(process.env.TELEGRAM_CHAT_ID || "", process.env.VITE_TELEGRAM_CHAT_ID || "", process.env.REACT_APP_TELEGRAM_CHAT_ID || "", process.env.NG_TELEGRAM_CHAT_ID || "");
    if (envToken && envChat) {
...
export function sanitizeOrderPayload(rawBody) {
    let body = rawBody;
    ...
    const sanitizedItems = rawItems.map((item) => {
        ...
        const variantSize = sanitizeString(rawVariant?.size ?? rawVariant?.name ?? rawVariant?.label ?? rawVariant) ||
            "standard";
        const variantPriceSource = parseAmount(typeof rawVariant?.price === "object" && rawVariant?.price !== null
            ? rawVariant?.price?.amount ?? rawVariant?.price?.value
            : rawVariant?.price) ?? parseAmount(item?.price) ?? parseAmount(item?.unitPrice);
    ...
    const enrichedItems = filteredItems.map((item) => {
        ...
        const variantSize = sanitizeString(item.variant?.size ?? item.variant?.label ?? item.variant?.name) || "standard";
        const variantCurrency = sanitizeString(item.variant?.currency || body?.totals?.currency || extractCurrencyFromPrice(item.unitPrice));
        const variantPriceRaw = typeof item.variant?.price === "object" && item.variant?.price !== null
    ...
    const totalsItems = Number.isFinite(totalsItemsProvided) && totalsItemsProvided >= 0
        ? totalsItemsProvided
        : itemsQuantity;
    ...
    const subtotalFromItems = roundCurrency(enrichedItems.reduce((sum, item) => sum + (item?.lineTotal ?? 0), 0));
    ...
    const currencyCandidate = sanitizeString(body?.totals?.currency) ||
    ...
    const customerIdentifier = resolveCustomerId(body?.customerId, body?.customer?.customerId, customer.phone, customer.email);
    const customerFingerprint = customerIdentifier || customer.phone || customer.email || `cust-${id}`;
...
const sanitizeCurrencyLabel = (currency) => sanitizeString(currency) || "EGP";
const buildOrderConfirmationHtml = (order) => {
    ...
    const deliveryDate = sanitizeString(order.etaDate) || "We’ll share shipping updates as soon as your package ships.";
    const itemRows = order.items
        ...
        const unitPriceCandidate = typeof item.variant?.price === "number"
            ? item.variant.price
        ...
        const lineTotal = unitPrice !== null && Number.isFinite(quantity) ? unitPrice * quantity : null;
        const formattedUnitPrice = unitPrice !== null ? `${unitPrice.toFixed(2)} ${currencyLabel}` : sanitizeString(item.unitPrice) || "-";
        const formattedLineTotal = lineTotal !== null ? `${lineTotal.toFixed(2)} ${currencyLabel}` : "";
        return `
        <tr>
...
export async function createOrder(rawBody, repo, emailProvider) {
    ...
    if (!payload.customer.name || !payload.customer.phone) {
        const error = new Error("Missing customer contact information.");
    ...
    if (payload.items.length === 0) {
        const error = new Error("Missing order items.");
        error.statusCode = 400;
        throw error;
    }
    const now = new Date();
    ...
    if (payload.paymentMethod === "cash_on_delivery" && payload.customer.phone) {
        ...
        if (duplicate) {
            const error = new Error("A recent order was already placed from this phone number. Please wait a moment.");
...
export async function updateOrderStatus(id, status, repo) {
    const store = repo ?? (await resolveOrdersRepository()).store;
...
export async function notifyTelegramTest() {
    const { token, chatId } = getTelegramConfig();
    if (!token || !chatId) {
        const error = new Error("Missing TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID env vars.");
        error.statusCode = 400;
        throw error;
    }
    const sampleOrder = {
    ...
    if (!result?.ok) {
        const err = new Error("Failed to send Telegram message. Check server logs for details.");
        err.statusCode = 500;
        throw err;
    }
    return { ok: true };
